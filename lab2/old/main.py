...
def alt_init_board():
    ...
    board = np.array([[EMP, EMP, EMP, EMP, EMP, EMP, EMP, EMP],
                      [EMP, EMP, EMP, MIN, EMP, EMP, EMP, EMP],
                      [EMP, EMP, EMP, EMP, MIN, EMP, EMP, EMP],
                      [EMP, EMP, EMP, MIN, MAX, MIN, EMP, EMP],
                      [EMP, EMP, EMP, MAX, MAX, MAX, EMP, EMP],
                      [EMP, EMP, EMP, EMP, MAX, MIN, EMP, EMP],
                      [EMP, EMP, EMP, EMP, EMP, EMP, EMP, EMP],
                      [EMP, EMP, EMP, EMP, EMP, EMP, EMP, EMP]])
    return board
...
def min_move(board):
    moves = find_all_moves(board, MIN)
    if len(moves) == 0:
        print("User wins!")
        exit()
    selection = alpha_beta_cutoff(board, MIN, moves)
    return apply_move(board, selection)
...
# number of stones + number of moves + flippers of the biggest move
# new calc:
# total number of possible stones to flip + number of stones
# basic calc:
# most stones flipped
def estimate(board, turn):
    moves = find_all_moves(board, turn)
    if len(moves) == 0:
        return -1
    total_flippers = 0
    for m in moves:
        total_flippers += len(m.flippers)
    return total_flippers
    # moves = find_all_moves(board, turn)
    # if len(moves) == 0:
    #     return -1
    # total_flippers = 0
    # for m in moves:
    #     total_flippers += len(m.flippers)
    # my_stones = board[board == turn]
    # return total_flippers + len(my_stones)


def alpha_beta_cutoff(board, turn, moves):
    v = max_value(board, turn, 0, -np.inf, np.inf)
    for m in moves:
        next_board = np.copy(board)
        apply_move(next_board, m)
        minn = min_value(next_board, other_player(turn), 1, -np.inf, np.inf)
        if minn == v:
            return m
    print("didn't find move with proper value")
    exit()
...
def max_value(board, turn, depth, alpha, beta):
    if cutoff_test(depth):
        return estimate(board, turn)
    v = -np.inf
    moves = find_all_moves(board, turn)
    for m in moves:
        next_board = np.copy(board)
        v = max(v, min_value(apply_move(next_board, m), other_player(turn), depth+1, alpha, beta))
        if v >= beta:
            return v
        alpha = max(alpha, v)
    return v
...
def min_value(board, turn, depth, alpha, beta):
    ...
    for m in moves:
        next_board = np.copy(board)
        v = min(v, max_value(apply_move(next_board, m), other_player(turn), depth+1, alpha, beta))
        if v <= alpha:
            return v
        beta = min(beta, v)
    return v
...
def test():
    ...
    print("Estimate: " + str(estimate(board, MIN)))
