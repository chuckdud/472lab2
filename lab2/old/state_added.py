...
def min_move(board):
    if len(find_all_moves(board, MIN) == 0):
        print("User wins!")
        exit()
    moves = find_all_moves(board, MIN)
    tree = alpha_beta_cutoff(board, MIN, moves)
    # x, y = map(int, input().split())
...
def estimate(state):
    moves = find_all_moves(state.board, state.turn)
    if len(moves) == 0:
        return -1
    total_flippers = 0
    for m in moves:
        total_flippers += len(m.flippers)
    my_stones = state.board[state.board == state.turn]
    return total_flippers + len(my_stones)


def alpha_beta_cutoff(state, moves):
...
def max_value(state, depth, alpha, beta):
    ...
    for m in moves:
        ...
        v = max(v, min_value(State(apply_move(next_board, m), other_player(state.turn)), depth+1, alpha, beta))
        if v >= beta:
            return v
        alpha = max(alpha, v)
    return v
...
def min_value(state, depth, alpha, beta):
    ...
    for m in moves:
        next_board = np.copy(state.board)
        v = min(v, max_value(State(apply_move(next_board, m), other_player(state.turn)), depth+1, alpha, beta))
        if v <= alpha:
            return v
        beta = min(beta, v)
    return v
