...
# TODO
def min_move(board):
    def alpha_beta_cutoff(turn):
        v = max_value(board, turn, -np.inf, np.inf)
        for m in moves:
            if m.value == v:
                return m
    ...
    # TODO
    def max_value(board2, turn, depth, alpha, beta):
        if cutoff_test(depth):
            return estimate(board2, turn)
        v = -np.inf
        moves = find_all_moves(board2, turn)
        for m in moves:
            next_board = np.copy(board2)
    ...
    # TODO
    def min_value(board2, turn, depth, alpha, beta):
        if cutoff_test(depth):
            return estimate(board2, turn)
        v = np.inf
        moves = find_all_moves(board2, turn)
    ...
    if len(moves) == 0:
        print("User wins!")
        exit()
    selection = alpha_beta_cutoff(MIN)
    # x, y = map(int, input().split())
